...
import { Position, type TextDocument, Uri, window, workspace } from "vscode";
 ...
 */
export async function createOrEditDocumentation(
	...
	if (uri) {
		const parent = Uri.file(path.dirname(uri.fsPath));
		...
		const snippetPathConfig =
			configuration.get().documentationCustomSnippetPath;
		if (!exists && snippetConfig !== "") {
			const snippet = await readDocumentationSnippet(
				snippetPathConfig,
				snippetConfig,
			);
			if (snippet) {
