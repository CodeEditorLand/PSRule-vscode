...
export class ConfigurationManager {
	...
	private loadSettings(): void {
		...
		this.current.documentationSnippet =
			config.get<string>("documentation.snippet") ??
			this.default.documentationSnippet;

		this.current.documentationPath =
			config.get<string>("documentation.path") ??
			this.default.documentationPath;

		this.current.documentationLocalePath =
			config.get<string>("documentation.localePath") ??
			this.default.documentationLocalePath;
		...
		this.current.executionRuleExcluded =
		...
		this.current.executionUnprocessedObject =
			config.get<ExecutionActionPreference>(
				...
				this.default.executionUnprocessedObject,
			);

		this.current.outputAs = config.get<OutputAs>(
			"output.as",
			this.default.outputAs,
		);
