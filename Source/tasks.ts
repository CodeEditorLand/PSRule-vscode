...
import { ExecutionActionPreference, configuration } from "./configuration";
...
export class PSRuleTaskProvider implements vscode.TaskProvider {
	...
	private createTask(
		...
		const executionRuleSuppressed =
			configuration.get().executionRuleSuppressed;
		...
		if (
			executionRuleExcluded !== undefined &&
		...
		if (
			executionRuleSuppressed !== undefined &&
		...
		if (
			executionUnprocessedObject !== undefined &&
			executionUnprocessedObject !== ExecutionActionPreference.None
		) {
			taskEnv.PSRULE_EXECUTION_UNPROCESSEDOBJECT =
