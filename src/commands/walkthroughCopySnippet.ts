 ...
 */
export async function walkthroughCopySnippet(
	...
	snippets.then((value): void => {
		value.forEach(
			(element: { name: string; snippet: string[] | undefined }) => {
				if (name === element.name && element.snippet) {
					logger.verbose(
						`Copying snippet for ${name} to the clipboard.`,
					);
...
async function load(name: string): Promise<any> {
	...
	const helpFile = info
		? path.join(info.path, `media/walkthroughs/${name}/snippets.json`)
		: undefined;
